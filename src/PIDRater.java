...
public class PIDRater {
	...
	public static double accuracyScore, timeOscillationScore, currentScore,
			overallScore;
	...
	/*
	 * The three criteria we will use will be as follows: -Time Oscillation
	 * (Integral of Time-Oscillation graph) -Current (Integral of Amps-Secs
	 * graph) -Accuracy (At finish, absolute value of distance from percent
	 * error 0)
	 * 
	 * Stops: -10 seconds (Safety) -100+ units of absolute value of PID Error
	 * -Over 100 amps of current at any given time -Over 20 amps of current
	 * after 5 seconds
	 */
	public static void checkStop() {
		...
		if (Robot.hammerTalon.getOutputCurrent() >= PIDMap.MAXIMUM_SUSTAIN_CURRENT
				&& Robot.time.get() >= PIDMap.CURRENT_CHECK_TIME) {
	...
	public static void getValues() {
		if (!getValueDone) {
			timeOscillation += Robot.time.get()
					* Math.abs(Robot.pid.getError());
			...
			boolean a = currentAccuracy < lastAccuracy
					+ PIDMap.PID_ERROR_THRESHOLD;
			boolean b = currentAccuracy > lastAccuracy
					- PIDMap.PID_ERROR_THRESHOLD;
			boolean c = currentAccuracy < secondToLastAccuracy
					+ PIDMap.PID_ERROR_THRESHOLD;
			boolean d = currentAccuracy > secondToLastAccuracy
					- PIDMap.PID_ERROR_THRESHOLD;

			if (a && b) {
				if (c && d) { // Inside both deadzones -> three consecutive
								// values within 1
					getValueDone = true;
					accuracy = Math
							.abs((currentAccuracy + lastAccuracy + secondToLastAccuracy) / 3.0);
					// Average of errors shows accuracy. Absolute value is
					// because accuracy isn't negative.
				}
			}
	...
	public static void processValues() { // Called constantly; Need error
											// handling?
		...
		// Current is scaled into a percentage relative to the maximum and them
		// rated from 0-100
		// Time Oscillation is also scaled to max and rated 0-100
	}

	public static double ratePID() {
		overallScore = (accuracyScore + currentScore + timeOscillationScore) / 3.0; // Average
																					// of
																					// three
																					// scores
																					// ->
		...
		SmartDashboard
				.putNumber("Time Oscillation Score", timeOscillationScore);
